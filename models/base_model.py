        ...
        if kwargs:
            for spec, value in kwargs.items():
                if spec == "__class__":
                    ...
                    setattr(self, spec, datetime.datetime.strptime(value, timePres))
                else:
                    setattr(self, spec, value)

    def __str__(self):
        """Returns user object in string format"""
        classname = f"[<{self.__class__.__name__}>({self.id})] {self.__dict__}"
        return classname
